...
select *,
FIRST_VALUE(EmployeeName) over(order by salary asc) [Emp Minimum Salary]
from EmployeeSalaries
...
select *
,FIRST_VALUE(EmployeeName) over(partition by department order by salary) [First Value Name]
from EmployeeSalaries