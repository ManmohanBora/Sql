...
select *,ROW_NUMBER() over(partition by subject order by marks desc) [Row_Number]
from Students

select *,ROW_NUMBER() over(partition by subject order by marks) [Row_Number]
from Students

select *,ROW_NUMBER() over(partition by student_name order by marks desc) [Row_Number]
from Students
...
select *,dense_rank() over(partition by subject order by marks desc) [Dense_Rank]
from Students